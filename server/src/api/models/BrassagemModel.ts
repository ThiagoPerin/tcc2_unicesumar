...
export class BrassagemModel {
	static getBrassagem(
		...
		pool.query(
			`SELECT * FROM ${process.env["MYSQL_DB"] as string}.BRASSAGEM WHERE NUM_OPERACAO = ? AND DELETED_AT IS NULL;`,
			[numOperacao],
			callback
		);
	}
	...
	static deleteBrassagem(
		...
		callback: (error: any, results: any) => void) {
		pool.query(
			`UPDATE ${process.env["MYSQL_DB"] as string}.BRASSAGEM SET DELETED_AT = CURDATE() WHERE id = ?;`,
			[id],
			callback
		);
	}
