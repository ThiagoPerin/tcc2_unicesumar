<script>
	...
	let dataCarbonatacao = (new Date()).toISOString().split('T')[0];
	let tanque, temperaturaInicial, temperaturaFinal, horaInicio, horaFim, pressao, responsavel, observacoes;
	...
	async function addRegister(e) {
		const data = {
			numOperacao: odp,
			dataCarbonatacao: dataCarbonatacao,
			tanque: tanque,
		...
		e.preventDefault();
		insertResult = await CarbonatacaoFetches.addCarbonatacao(data);
...
<form on:submit={(e) => addRegister(e)}>
	<DefaultTitle titulo={`Carbonatação`}/>
	<div class="inputsArea">
		<div class="inputDiv">
			<label for="dataCarbonatacao" class="form-label">Data</label>
			<input type="date" class="form-control inputTag" id="dataCarbonatacao" bind:value={dataCarbonatacao} required/>
		</div>
		<div class="inputDiv">
		...
		<div class="inputDiv">
			<label for="temperaturaInicial" class="form-label">Temperatura início (°C)</label>
			<input type="number" class="form-control inputTag" id="temperaturaInicial" step=".001" bind:value={temperaturaInicial} required/>
		</div>
		<div class="inputDiv">
			<label for="temperaturaFinal" class="form-label">Temperatura fim (°C)</label>
			<input type="number" class="form-control inputTag" id="temperaturaFinal" step=".001" bind:value={temperaturaFinal} required/>
		</div>
	</div>
	<div class="inputsArea">
		<div class="inputDiv">
		...
		<div class="inputDiv">
			<label for="pressao" class="form-label">Pressão (Kgf)</label>
			<input type="number" class="form-control inputTag" id="pressao" step=".001" bind:value={pressao} required/>
		</div>
		<div class="inputDiv">
	...
	<div class="inputsArea">
		<div class="textareaDiv">
			...
			<textarea class="form-control" id="observacoes" bind:value={observacoes} rows="3" maxlength="100" placeholder="detalhes da carbonatação..."></textarea>
		</div>
	</div>
	<div class="inputsArea">
